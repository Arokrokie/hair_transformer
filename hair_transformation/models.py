...
class HairTransformation(models.Model):
    ...
    # Processed images
    hair_analysis_image = models.ImageField(
        upload_to=result_upload_path, null=True, blank=True
    )
    face_analysis_image = models.ImageField(
        upload_to=result_upload_path, null=True, blank=True
    )
    hair_mask_image = models.ImageField(
        upload_to=result_upload_path, null=True, blank=True
    )
    hair_coverage = models.FloatField(null=True, blank=True)
    # Progress tracking
    progress = models.IntegerField(default=0)
    processing_step = models.CharField(max_length=200, blank=True)
...
class TransformationResult(models.Model):
    hair_transformation = models.ForeignKey(
        HairTransformation, on_delete=models.CASCADE, related_name="results"
    )
